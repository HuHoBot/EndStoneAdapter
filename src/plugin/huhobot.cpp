...
// 添加单例访问方法
HuHoBot& HuHoBot::getInstance() { return *instance_; }
string   HuHoBot::getVersion() { return version; }

BotClient& HuHoBot::getBotClient() { return *client; }

void HuHoBot::onLoad() { getLogger().info("HuHoBot v" + version + " loaded."); }

void HuHoBot::onEnable() {
    client = std::make_shared<BotClient>(&getLogger());
...
void HuHoBot::onDisable() { client->DestoryClient(); }
...
void HuHoBot::broadcast(const std::string& msg) { this->getServer().broadcastMessage(msg); }
...
std::pair<string, bool> HuHoBot::runCommand(const std::string& cmd) {
    ...
    CommandSenderWrapper sender_wrapper(
        ...
        // 正确捕获 Message 类型参数
        [&msgRet, this](const endstone::Message& msg) { msgRet += "\n" + getMessageContent(msg); },
...
std::vector<Player*> HuHoBot::getOnlinePlayers() { return this->getServer().getOnlinePlayers(); }

std::shared_ptr<endstone::Task> HuHoBot::setReconnectTask() {
    return this->getServer().getScheduler().runTaskTimer(*this, [&]() { client->task_reconnect(); }, 0, 5 * 20);
}
...
std::shared_ptr<endstone::Task> HuHoBot::setHeartTask() {
    return this->getServer().getScheduler().runTaskTimer(*this, [&]() { client->sendHeart(); }, 0, 5 * 20);
}

void HuHoBot::broadcastMsg(const string& msg) {
    getServer().getScheduler().runTask(*this, [this, msg]() { broadcast(msg); });
}
